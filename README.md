...
- 默认每 30 分钟扫描一次所有已加载世界中的掉落物实体。
- 默认在清扫前 5 分钟、60 秒、30 秒和 10 秒分别发送全服提醒。
...
插件首次启用后，第一次清扫会按照配置的清扫间隔安排。玩家也可以使用 `/mcgimtrash` 打开共享垃圾桶。
...
插件首次启动时会生成带有中文注释的 `plugins/mcgimtrash/config.yml`：

| 配置项 | 作用 | 默认值 |
| --- | --- | --- |
| `sweep-interval-minutes` | 自动清扫间隔，单位为分钟 | `30` |
| `sweeps-per-cycle` | 一个垃圾桶周期包含的清扫次数 | `10` |
| `reminder-seconds` | 每次清扫前发送提醒的剩余秒数列表 | `300, 60, 30, 10` |
| `message-prefix` | 全服提醒和清扫结果消息的前缀 | `[GIM]` |

- `/mcgimtrash`：玩家打开共享垃圾桶。
- `/mcgimtrash reload`：重新读取并应用配置，控制台或拥有 `mcgimtrash.admin` 权限的管理员可用。

重载不会清空垃圾桶或重置清扫周期。若新的清扫间隔短于当前剩余时间，下一次清扫会相应提前；提醒状态会按新配置重新计算。配置内容无效时，重载会失败并继续使用原有运行参数，具体原因会写入控制台日志。
